...
$countUso = $mysql->getValue('count(id) as total', 'total', 'aulaspraticas', "idcarrofuncionario = '".$id."'");

if ($countUso > 0) {
    $msgValidacao["msg"][] = "Este Carro x Funcionário já foi usado em um aula prática e não pode ser excluído.";
    $validacao = false;
}

if ($validacao) {
    if ($mysql->delete('carrofuncionario', "id = '".$id."'")) {
