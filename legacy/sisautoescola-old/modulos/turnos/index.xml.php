...
$countTable = $mysql->getValue('count(a.id) as total','total','turnos a, tipocarros b',$where);

//echo $mysql->getMsgErro()."<br>";
$xmlMainFun = new modulos_global_gridxml($page, $limit, $countTable);

$rows = $mysql->select('a.id, a.descricao, a.duracaoaula, b.descricao as tipo', 'turnos a, tipocarros b', $where, "order by $sidx $sord LIMIT ".$xmlMainFun->getStart()." , ".$xmlMainFun->getLimit()." ");
//echo $mysql->getMsgErro()."<br>";
...
if (is_array($rows)) {
    foreach ($rows as $row) {
        ...
        $xmlMainFun->addCell($row["duracaoaula"]);
        $xmlMainFun->endRow();
        $xCount++;
    }
}
