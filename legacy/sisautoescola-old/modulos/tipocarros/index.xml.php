...
$countTable = $mysql->getValue('count(id) as total','total','tipocarros',$where);

//echo $mysql->getMsgErro()."<br>";
$xmlMainFun = new modulos_global_gridxml($page, $limit, $countTable);

$rows = $mysql->select('id, descricao, comissao', 'tipocarros', $where, "order by $sidx $sord LIMIT ".$xmlMainFun->getStart()." , ".$xmlMainFun->getLimit()." ");
//echo $mysql->getMsgErro()."<br>";
...
if (is_array($rows)) {
    foreach ($rows as $row) {
        ...
        $xmlMainFun->addCell(db_to_float($row["comissao"]));
        $xmlMainFun->endRow();
        $xCount++;
    }
}
